...
class ItemSectionRecyclerView(
    ...
    override fun getItemOffsets(
        ...
        if (currentModel.updatedAt != previousModel.updatedAt)
            outRect.top = sectionItemHeight
        else {
    ...
    override fun onDraw(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        for (i in 0 until childCount) {
            ...
            val curPosDate = itemModel.updatedAt

            if (getItemList().isNotEmpty() && (0 == position || curPosDate != getItemList()[position - 1].updatedAt)) {

                val top = childView.top - sectionItemHeight
                drawSectionView(c, curPosDate.toString(), top)
